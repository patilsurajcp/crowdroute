...
const TRANSPORT_ICONS = {
  bus: '🚌', metro: '🚇', train: '🚆',
  chigari: '⚡', ferry: '⛴️', tram: '🚋',
  toy_train: '🚂', shared_cab: '🚖', shikara: '🛶'
}
    ...
    <div className={`relative rounded-2xl border-2 ${style.border} ${style.bg} p-5 transition-all duration-300`}>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="font-bold text-gray-800 capitalize">{result.transport}</p>
            {result.estimated_time && (
              <p className="text-xs text-gray-500">⏱️ ~{result.estimated_time}</p>
            )}
          </div>
        </div>
      ...
      {result.route_note && (
        <p className="text-xs text-gray-500 mb-2">🗺️ {result.route_note}</p>
      )}
      {/* Replace this */}
      {result.estimated_time && (
      <p className="text-xs text-gray-500">⏱️ ~{result.estimated_time}</p>
      ...
      {/* With this */}
      {result.estimated_time && result.estimated_time !== 'N/A' && (
        <p className="text-xs text-gray-500">⏱️ ~{result.estimated_time}</p>
      )}
